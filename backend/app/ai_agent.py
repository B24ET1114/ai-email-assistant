...
- works_for: list of participants it works for
...
Generate a concise agenda with 3-5 bullet points covering what should be discussed.
Return only the agenda text, nothing else.
"""
    response = client.chat.completions.create(
        model="llama-3.1-8b-instant",
        messages=[{"role": "user", "content": prompt}],
    ...
    return response.choices[0].message.content.strip()
def detect_ambiguity(email_body: str) -> dict:
    prompt = f"""
Analyze this email for scheduling ambiguity.
Return a JSON object with:
- is_ambiguous: true or false
- reason: why it is ambiguous (empty string if not)
- clarification_question: what to ask to resolve ambiguity (empty string if not ambiguous)
